...
SUBJECT       : Implementation of a chat server.
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    if (argc != 4) {
      cerr << "Usage: " << argv[0] << " nickname NameService-host local-port\n";
      goto clean_up;
    }
    ...
    Ice::ObjectPtr object = new ChatDisplay;
    Ice::ObjectPrx serverObject =
      adapter->add(object, ic->stringToIdentity("ChatWindow"));
    TextSinkPrx server = TextSinkPrx::checkedCast(serverObject);
    adapter->activate();
